...
public final class ValidationCheck {
    ...
    /**
     * Two markers carrying the same marker id.
     *
     * What a WorldEdit copy-paste of part of a building produces: the id is stamped onto the block
     * when it is placed, so anything that duplicates the block duplicates the id. A warning rather
     * than a refusal — nothing addresses a marker by id at runtime, so the structure still works, but
     * the id stops being the unique handle every log line and stored record assumes it is.
     */
    private static void duplicateMarkerIds() {
        List<PlacedMarker> markers = complete();

        WorldPoint first = new WorldPoint("world", 4.5d, 64d, 4.5d, 0f, 0f);
        WorldPoint second = new WorldPoint("world", 5.5d, 64d, 5.5d, 0f, 0f);

        markers.add(new PlacedMarker("copied-id", "npc_slot_3", 1, first, Map.of()));
        markers.add(new PlacedMarker("copied-id", "npc_slot_4", 1, second, Map.of()));

        List<MarkerProblem> problems = validate(markers);

        check("a duplicate marker id warns", has(problems, "duplicate-marker-id", false));
        check("a duplicate marker id is not fatal", fatal(problems).isEmpty());

        // Distinct ids on the same two types must stay silent, or the rule would fire on every
        // structure that legitimately has more than one NPC slot.
        List<PlacedMarker> distinct = complete();

        distinct.add(new PlacedMarker("slot-a", "npc_slot_3", 1, first, Map.of()));
        distinct.add(new PlacedMarker("slot-b", "npc_slot_4", 1, second, Map.of()));

        check("distinct marker ids are silent",
                !has(validate(distinct), "duplicate-marker-id", false));

        // Blank ids are what a marker restored from an older stored record has. Several of them must
        // not be reported as sharing an identity, because they have none.
        List<PlacedMarker> blank = complete();

        blank.add(new PlacedMarker("", "npc_slot_3", 1, first, Map.of()));
        blank.add(new PlacedMarker("", "npc_slot_4", 1, second, Map.of()));

        check("blank marker ids are not duplicates",
                !has(validate(blank), "duplicate-marker-id", false));
    }
