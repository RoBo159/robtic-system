...
import org.robtic.essentials.model.SurvivalModels.AfkTotals;
...
public final class ProfileMenu {
    ...
    /** Null when RobticMail is not installed, in which case the mailbox button is not drawn. */
    private final MailboxService mailbox;
    private final AfkService afk;
    private final AfkRewardService afkRewards;

    public ProfileMenu(MessageCatalog messages, MailboxService mailbox, AfkService afk, AfkRewardService afkRewards) {
    ...
    public void open(Player viewer, Profile profile, Homes ownHomes) {
        ...
        // Also skipped when RobticMail is absent: a button that cannot open anything is worse than
        // no button, because a player will click it and conclude their mail is broken.
        if (ownProfile && mailbox != null) {
            int unread = mailbox.unreadCount(viewer.getUniqueId());
    ...
    private List<String> afkLore(Profile profile) {
        ...
        lore.add(messages.text("afk.profile-robs", "robs", org.robtic.core.util.Robs.format(totals.robs())));
        ...
        if (isAfk) {
            ...
            lore.add(messages.text("afk.profile-earning",
                    "robs", org.robtic.core.util.Robs.format(afkRewards.projectedRobs(session))));
        }
