...
public final class SigCheck {
    ...
    public static void main(String[] a) throws Exception {
        ...
        // A real temporary directory, deleted on the way out.
        //
        // This used to be a hard-coded "D:/tmp/lic/keys/licenses.key". On Windows that is an
        // absolute path and behaves; inside the Linux container this actually runs in, "D:" is just
        // a directory name, so the whole thing was RELATIVE — and every run created a folder called
        // "D:" in the working directory and left a live HMAC signing key inside it, in the source
        // tree, not covered by .gitignore.
        //
        // Nothing about the test needed a fixed location. It needs a path that does not exist yet.
        Path directory = Files.createTempDirectory("robtic-sigcheck");
        Path key = directory.resolve("licenses.key");

        try {
            run(log, key);
        } finally {
            // The key is a secret even in a test. Removed whether or not the checks passed, so a
            // failing run does not leave one behind for the next reader to wonder about.
            Files.deleteIfExists(key);
            Files.deleteIfExists(directory);
        }

        // Exits here and not inside run(), because System.exit terminates the JVM without running
        // finally blocks — which is the second half of why a key was being left behind. The cleanup
        // above has to happen first.
        System.exit(fail == 0 ? 0 : 1);
    }

    private static void run(Logger log, Path key) throws Exception {
        Files.deleteIfExists(key);
