...
public interface WorkspaceExtension {
    ...
    /**
     * A workspace upgrade was bought.
     *
     * Separate from {@link #onUpgraded}, which is the headquarters. The two are different decisions
     * with different consequences, and a listener that cared about storage growing should not also
     * fire when the building changed shape.
     *
     * @param upgradeId which upgrade
     * @param toLevel   the level it now stands at
     */
    default void onUpgradeBought(Workspace workspace, String upgradeId, int toLevel, Player owner) {
    }
