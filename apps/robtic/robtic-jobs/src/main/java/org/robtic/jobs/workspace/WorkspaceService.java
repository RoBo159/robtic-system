...
public final class WorkspaceService {
    ...
    public enum UpgradeResult {
        ...
        SUSPENDED,
        VETOED,
        /** A base level's own requirements — a reputation, a licence, a profession level — are unmet. */
        REQUIREMENTS_UNMET,
        /** A workspace upgrade needs a base level this business has not reached. */
        BASE_LEVEL_TOO_LOW,
        /** A workspace upgrade depends on another that is not high enough yet. */
        MISSING_DEPENDENCY,
        /** The upgrade id is not configured, or the base level does not unlock it. */
        UNAVAILABLE,
        SAVE_FAILED
    }
    ...
    /**
     * Core's attribute router, for evaluating a base level's requirements.
     *
     * Null on a server where Core did not publish one. Every requirement then counts as met, which is
     * the same direction {@link BaseLevel} takes when it cannot parse them: a business that cannot
     * be upgraded because an unrelated system is missing is a worse failure than an ungated upgrade.
     */
    private volatile org.robtic.core.unlock.Attributes attributes;

    /**
     * Replaces the building when a base level is reached. Optional; see {@code BuildingService}.
     *
     * A function rather than a field of the building service's type, so this package does not learn
     * that schematics — or the plugin that pastes them — exist.
     */
    private volatile BuildingPaster paster = BuildingPaster.NONE;

    /** How a base level's schematic reaches the world. Implemented outside this package. */
    @FunctionalInterface
    public interface BuildingPaster {

        /** Does nothing, for a server with no paste backend installed. */
        BuildingPaster NONE = (workspace, level, whenDone) -> whenDone.accept(false);

        /**
         * Pastes a level's building over the business's own.
         *
         * @param whenDone called on the main thread; false when nothing was pasted, which is an
         *                 ordinary outcome and never a reason to undo the upgrade
         */
        void paste(Workspace workspace, BaseLevel level, java.util.function.Consumer<Boolean> whenDone);
    }

    public void attributes(org.robtic.core.unlock.Attributes attributes) {
        this.attributes = attributes;
    }

    public void paster(BuildingPaster paster) {
        this.paster = paster == null ? BuildingPaster.NONE : paster;
    }
    ...
    public BaseLevel baseOf(Workspace workspace) {
        return settings.base(workspace.level());
    }
    ...
    public ClaimResult claim(
        ...
        return claim(owner, professionId, structureId, anchor, radius, Optional.empty(), whenClaimed);
    }

    /**
     * Claims a structure whose area the builder marked out.
     *
     * <h2>The marked region wins over the radius</h2>
     *
     * When RobticWorld scanned the structure, its origin and end markers define exactly what the
     * building is, and that is what should be protected. The radius is the fallback for the cases
     * where no such region is known — a recruiter recovered from its own tags after a restart, or a
     * profession granted by command — and it is a poor substitute: a box centred on wherever the
     * recruiter happened to stand is not the building unless the recruiter is at its centre, and is
     * never its shape.
     *
     * The volume and overlap checks run against whichever region was chosen, so a marked-out region
     * is held to the same limits as a generated one.
     *
     * @param marked the region the structure's corner markers describe, when one is known
     */
    public ClaimResult claim(
            Player owner,
            String professionId,
            ...
            WorldPoint anchor,
            int radius,
            Optional<WorkspaceRegion> marked,
            java.util.function.Consumer<Optional<Workspace>> whenClaimed
    ) {
        if (!settings.enabled()) {
        ...
        try {
            result = claimChecked(owner, professionId, structureId, anchor, radius, marked, whenClaimed);
    ...
    private ClaimResult claimChecked(
        ...
        // The marked region is padded, because the corner markers describe the building and the
        // protected area should include the ground it stands on and the air above it. Both figures
        // are the server's own, so an operator who wants protection to stop at the walls sets them
        // to zero rather than editing every schematic.
        WorkspaceRegion region = marked
                .map(box -> box.padded(settings.regionPadding(), settings.regionHeight()))
                .orElseGet(() -> WorkspaceRegion.around(anchor,
    ...
    /**
     * Raises a business by one base level.
     *
     * <h2>Atomic, and nothing a player owns is touched</h2>
     *
     * The player is charged only after every check has passed, the record is committed before the
     * world changes, and a failed save both refunds and restores the previous state. Storage,
     * upgrades and the licence snapshot carry across because {@link Workspace#withLevel} copies them
     * — there is deliberately no code path in an upgrade that can reach a player's items.
     *
     * That guarantee is what makes it safe for the building itself to be replaced afterwards: the
     * schematic paste happens last, against a record that is already committed, so a paste that
     * fails costs a building's appearance and never a business's data.
     *
    ...
    public void upgrade(Player player, Workspace workspace, java.util.function.Consumer<UpgradeResult> whenDone) {
        ...
        if (suspended(workspace)) {
            // Upgrading a suspended business would let a player buy their way past the one
            // consequence unpaid maintenance — or a lapsed licence — actually has.
            whenDone.accept(UpgradeResult.SUSPENDED);
            return;
        }

        Optional<BaseLevel> next = settings.nextBase(workspace.level());
        ...
        // Checked before the money, so a player who cannot meet a requirement is told why rather
        // than being charged and refunded.
        if (!meetsRequirements(target, player)) {
            whenDone.accept(UpgradeResult.REQUIREMENTS_UNMET);
            return;
        }
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            boolean paid = !org.robtic.core.util.Robs.isPositive(target.cost())
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                ...
                repository.put(upgraded, saved -> {
                    ...
                    if (!saved) {
                        ...
                        // money goes back, because the player was charged for a level they do not have.
                    ...
                    //
                    // The building is replaced before the NPCs are placed, because a paste covers the
                    // ground they stand on and would otherwise bury them. Staffing runs in the paste
                    // callback for the same reason — and unconditionally, so a server with no paste
                    // backend still gets its NPCs.
                    //
                    // A failed paste is deliberately not an error here. The record is already
                    // committed and correct; the player has the level they paid for, and the building
                    // simply still looks like the old one. Undoing a committed upgrade to keep the
                    // walls in step would be the far worse trade.
                    paster.paste(upgraded, target, pasted -> {
                        staffNpcs(upgraded);
     ...
     *
     * <h2>The same atomicity as a base-level upgrade, and for the same reason</h2>
     *
     * Validate, charge, re-read, commit, and on any failure restore and refund. The re-read matters
     * as much here as there: the payment crosses a network, and the workspace that comes back may
     * have gained items or had another upgrade bought on the tick meanwhile. Writing back the
     * snapshot this call started from would silently undo whichever happened.
     *
     * <h2>Why every gate is checked here rather than in the menu</h2>
     *
     * The menu greys out what cannot be bought, but the menu is a view. A player with a stale
     * inventory open, or a second click landing after the first, must not be able to buy a step whose
     * requirements stopped holding — so the rules live on the path that takes the money.
     *
     * @param whenDone called on the main thread with the outcome
     */
    public void buyUpgrade(
            ...
            Workspace workspace,
            String upgradeId,
            java.util.function.Consumer<UpgradeResult> whenDone
    ) {
        if (!workspace.ownedBy(player.getUniqueId()) && !player.hasPermission(BYPASS)) {
            whenDone.accept(UpgradeResult.NOT_OWNER);
            return;
        ...
        Optional<WorkspaceUpgrade> found = settings.upgrade(upgradeId);

        if (found.isEmpty()) {
            whenDone.accept(UpgradeResult.UNAVAILABLE);
            return;
        }

        WorkspaceUpgrade upgrade = found.get();
        BaseLevel base = settings.base(workspace.level());
        ...
        Optional<WorkspaceUpgrade.Step> nextStep = upgrade.next(current);
        ...
        WorkspaceUpgrade.Step step = nextStep.get();

        if (workspace.level() < step.minBaseLevel()) {
            whenDone.accept(UpgradeResult.BASE_LEVEL_TOO_LOW);
            return;
        }

        for (Map.Entry<String, Integer> dependency : step.requires().entrySet()) {
            if (workspace.upgradeLevel(dependency.getKey()) < dependency.getValue()) {
                whenDone.accept(UpgradeResult.MISSING_DEPENDENCY);
                return;
            }
        }

        // Keyed on the workspace, like a base upgrade, so the two cannot both be charging at once.
        if (!busy.add(workspace.id())) {
            whenDone.accept(UpgradeResult.SAVE_FAILED);
            return;
        }
        ...
        String name = player.getName();
        String workspaceId = workspace.id();
        String reference = "workspace-upgrade:" + workspaceId + ":" + upgrade.id() + ":" + step.level();

        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            boolean paid = !org.robtic.core.util.Robs.isPositive(step.cost())
                    || economy.pay(playerId, name, -step.cost(), reference);
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                ...
                if (latest.isEmpty()) {
                    busy.remove(workspaceId);
                    refund(playerId, name, step.cost(), workspaceId);
                    whenDone.accept(UpgradeResult.SAVE_FAILED);
                    return;
                }
                ...
                // Re-read rather than trusted: another click may have bought this same step while the
                // payment was in flight. Setting the level absolutely from what is stored now means a
                // lost race costs a refund rather than two levels for one purchase.
                if (previous.upgradeLevel(upgrade.id()) >= step.level()) {
                    busy.remove(workspaceId);
                    refund(playerId, name, step.cost(), workspaceId);
                    whenDone.accept(UpgradeResult.SAVE_FAILED);
                    return;
                ...
                Workspace bought = previous.withUpgrade(
                        upgrade.id(), step.level(), System.currentTimeMillis());

                repository.put(bought, saved -> {
                    busy.remove(workspaceId);
                    ...
                    // An upgrade can change which NPC roles make sense — a market stall staffs a
                    // trader — so the staffing pass runs here too. It is idempotent and does nothing
                    // for the common case.
                    staffNpcs(bought);

                    notifyExtensions(extension ->
                            extension.onUpgradeBought(bought, upgrade.id(), step.level(), player));
    ...
    /**
     * Commits a change, putting the previous state back if the write does not land.
     *
     * The put-then-rollback pattern every atomic operation here uses, in one place. It exists mostly
     * for the sub-packages: {@code WorkspaceRepository#rollback} is package-private on purpose — it
     * repairs the in-memory index and is not something arbitrary code should reach — but the worker
     * system genuinely does need to commit a change it can undo, and widening the repository for
     * that would open it to everything else too.
     *
     * @param whenDone called on the main thread; false when the change did not persist, in which
     *                 case the caller must undo whatever else it did (refund, despawn, unlock)
     */
    public void persist(Workspace previous, Workspace next, java.util.function.Consumer<Boolean> whenDone) {
        repository.put(next, saved -> {
            if (!saved) {
                repository.rollback(previous);
            }

            whenDone.accept(saved);
        });
    }
    ...
    /**
     * Whether this business is currently stopped from trading, for any reason.
     *
     * The two causes — unpaid tax and a lapsed workspace licence — have identical effects, and every
     * caller that asks "may this business do something" wants both. Keeping the question in one place
     * is what stops a new feature being gated on tax alone and quietly working without a licence.
     */
    public boolean suspended(Workspace workspace) {
        return workspace.taxSuspended()
                || workspace.licenseLapsed(System.currentTimeMillis());
    }

    /**
     * Whether a player satisfies a base level's non-monetary requirements.
     *
     * Answers true when Core published no attribute router: see {@link #attributes}.
     */
    public boolean meetsRequirements(BaseLevel level, Player player) {
        if (level.requirements().isEmpty()) {
            return true;
        }

        org.robtic.core.unlock.Attributes router = attributes;

        return router == null || level.requirementsMet(org.robtic.core.unlock.UnlockContext.of(
                player.getUniqueId(), Optional.of(player), router));
    }

    /** The first requirement a player does not meet, for telling them why. */
    public Optional<String> unmetRequirement(BaseLevel level, Player player) {
        org.robtic.core.unlock.Attributes router = attributes;

        if (router == null || level.requirements().isEmpty()) {
            return Optional.empty();
        }

        return level.firstUnmet(org.robtic.core.unlock.UnlockContext.of(
                player.getUniqueId(), Optional.of(player), router));
    }
    ...
    private Workspace restaff(Workspace workspace) {
        ...
        boolean suspended = suspended(workspace);
        ...
        for (WorkspaceNpcRole role : settings.roles().all()) {
            ...
            boolean wanted = base.staffs(role.id()) && !(suspended && role.suspendable());
    ...
    /**
     * How many items this business can hold.
     *
     * The configured floor plus whatever the storage upgrade has been bought to. Capacity moved off
     * the base level deliberately: a player who wants more room should be able to buy room, rather
     * than being made to buy a headquarters they have no other use for.
     *
     * A business that has never bought any storage still has the floor, so nothing that existed
     * before the upgrade axis stops working.
     */
    public int capacityOf(Workspace workspace) {
        double bought = settings.upgrade(settings.storageUpgradeId())
                .map(upgrade -> upgrade.valueAt(workspace.upgradeLevel(upgrade.id())))
                .orElse(0d);

        return settings.baseStorage() + (int) Math.max(0d, bought);
    }
