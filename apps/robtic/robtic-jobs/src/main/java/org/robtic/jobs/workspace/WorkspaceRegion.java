...
public record WorkspaceRegion(
    ...
    /**
     * Grows a region outward.
     *
     * Used on a region derived from a structure's corner markers: those describe the building, and
     * the protected area wants a margin of ground around it and headroom above it. Vertical growth is
     * upward only — a workspace should not silently protect the bedrock under somebody's mine.
     *
     * Negative input is clamped to zero rather than shrinking the region, because a padding that ate
     * into the building would protect less than the builder marked out and nothing about the name
     * suggests it could.
     */
    public WorkspaceRegion padded(int horizontal, int up) {
        int out = Math.max(0, horizontal);

        return new WorkspaceRegion(world,
                minX - out, minY, minZ - out,
                maxX + out, maxY + Math.max(0, up), maxZ + out);
    }
