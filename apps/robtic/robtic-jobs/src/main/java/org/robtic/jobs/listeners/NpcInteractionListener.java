...
import org.robtic.jobs.workspace.StructureMarker;
 ...
 * a stylistic preference: a packet-based backend has no entity, so a
 * {@code PlayerInteractEntityEvent} handler would silently never fire for them. Normalising the
 * click at the provider means this class is written once and works on every backend.
 *
...
public final class NpcInteractionListener {
    ...
    private void recruit(Player player, NpcHandle handle) {
        ...
        if (result != JobService.ClaimResult.SUCCESS) {
            player.sendMessage(messages.prefixed(refusalKey(result),
                    "job", job.get().display(),
                    // The licence a refusal is about. Blank for every other refusal, which never
                    // reads it — a placeholder that is not in the message costs nothing.
                    "license", job.get().license().orElse(""),
    ...
    private void claimWorkspace(Player player, Job job, StructureMarker marker) {
        ...
        WorkspaceService.ClaimResult result = workspaces.claim(
                ...
                // What the builder marked out with the origin and end markers, when this structure
                // was scanned this session. Empty for a recruiter recovered from its own tags after
                // a restart, which falls back to the radius — see WorkspaceService#claim.
                discovery.regionOf(marker.structureId()),
                claimed -> {
                    if (claimed.isEmpty()) {
    ...
    private static String refusalKey(JobService.ClaimResult result) {
        return switch (result) {
            ...
            case LICENSE_MISSING -> "progression.jobs.license-missing";
            case LICENSE_EXPIRED -> "progression.jobs.license-expired";
            case LICENSE_REFUSED -> "progression.jobs.license-refused";
