...
import org.robtic.minecraft.progression.npc.providers.BuiltinNpcProvider;
import org.robtic.minecraft.progression.npc.providers.CitizensNpcProvider;
import org.robtic.minecraft.progression.npc.providers.FancyNpcsProvider;
 ...
 *
 * <pre>
 *   auto        FancyNPCs, then Citizens, then builtin — first one usable wins
 *   fancynpcs   FancyNPCs, falling back to builtin if it is not installed
 *   citizens    Citizens, falling back to builtin
 *   builtin     always the plain-entity implementation
 * </pre>
 *
 * FancyNPCs is preferred in {@code auto} because its NPCs are packets: no mob cap, no entity ticks,
 * nothing to accidentally kill. Citizens is second because it is the one most servers already run.
 * The builtin is last and always available, which is what stops either plugin being a prerequisite.
 *
 * A configured backend that is not installed <em>falls back</em> rather than disabling NPCs. An
 * operator who names FancyNPCs and has not installed it has made a mistake in the config; taking
...
public final class NpcBackend {
     ...
     *
     * @param preference {@code auto}, {@code fancynpcs}, {@code citizens} or {@code builtin}
     */
    public static NpcBackend create(Plugin plugin, String preference) {
        ...
        FancyNpcsProvider.createIfPresent(plugin).ifPresent(available::add);
        CitizensNpcProvider.createIfPresent(plugin).ifPresent(available::add);

        // Always constructed, and always last. It is the reason neither plugin is required.
        BuiltinNpcProvider builtin = new BuiltinNpcProvider(plugin);
        available.add(builtin);

        NpcProvider chosen = switch (wanted) {
            case "fancynpcs", "fancy" -> named(available, FancyNpcsProvider.NAME, builtin, plugin, wanted);
            ...
            default -> {
                if (!wanted.equals("auto")) {
                    plugin.getLogger().warning("npc.yml names the unknown backend \"" + wanted
                            + "\". Valid values are auto, fancynpcs, citizens, builtin. Using auto.");
                }
     ...
     *
     * Asked of every provider, because a server that switched from Citizens to FancyNPCs still has
     * Citizens NPCs standing in old workspaces and clicking one should still work.
     */
