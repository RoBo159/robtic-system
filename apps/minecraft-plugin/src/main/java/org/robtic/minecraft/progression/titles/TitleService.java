...
import org.robtic.minecraft.progression.api.UnlockContext;
import org.robtic.minecraft.progression.events.PlayerSelectTitleEvent;
import org.robtic.minecraft.progression.events.PlayerUnlockTitleEvent;
import org.robtic.minecraft.progression.storage.PlayerProgression;
...
public final class TitleService {
    ...
    public PlayerTitles titlesOf(UUID playerId) {
        return repository.get(playerId).titles();
    }
    ...
    public boolean unlock(UUID playerId, String titleId, String reason) {
        ...
        repository.mutateAndSave(playerId,
                progression -> progression.withTitles(progression.titles().withOwned(title.id())));
    ...
    public boolean revoke(UUID playerId, String titleId) {
        if (!repository.isLoaded(playerId) || !titlesOf(playerId).owns(titleId)) {
            return false;
        }
        ...
        repository.mutateAndSave(playerId,
                progression -> progression.withTitles(progression.titles().withoutOwned(titleId)));
    ...
    public Optional<Refusal> equip(UUID playerId, String titleId) {
        ...
        repository.mutateAndSave(playerId,
                progression -> progression.withTitles(progression.titles().equipping(title.id())));
    ...
    public boolean unequip(UUID playerId) {
        if (!repository.isLoaded(playerId) || titlesOf(playerId).equipped().isEmpty()) {
            return false;
        }
        ...
        repository.mutateAndSave(playerId,
                progression -> progression.withTitles(progression.titles().unequipped()));
