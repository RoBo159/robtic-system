...
public final class BridgeConsumerService {
    ...
    public BridgeConsumerService(
            ...
            JailService jail,
            /** Null when RobticAuth is disabled, in which case its events are simply ignored. */
            org.robtic.minecraft.auth.AuthService auth
    ) {
        this.plugin = plugin;
        ...
        handlers.put("staff_chat", payload -> onMain(() ->
                staffChat.showFromDiscord(string(payload, "username"), string(payload, "message"))));
        ...
        handlers.put("jail_release", payload -> {
            ...
            if (uuid != null) {
                onMain(() -> jail.applyRemoteState(uuid, false, null));
            }
        });
        ...
        handlers.put("freeze_release", payload -> {
            UUID uuid = uuid(payload, "minecraftUuid");
            if (uuid != null) {
                onMain(() -> freeze.applyRemoteState(uuid, false, null));
            }
        });

        // RobticAuth. Registered only when the feature is on, so a server with authentication
        // disabled ignores these exactly as it ignores any other unknown type.
        if (auth != null) {
            handlers.put("account_linked", payload -> {
                UUID uuid = uuid(payload, "minecraftUuid");
                if (uuid != null) {
                    onMain(() -> auth.onLinked(
                            uuid,
