package org.robtic.minecraft.model.survival;
...
public final class SurvivalModels {
     ...
     *
     * Every limit the plugin enforces is read from here rather than from a config constant, so a
     * tier change on Discord is reflected without a plugin edit.
     */
    public record Entitlements(
         ...
         * premium sync will not grant them a tier's LuckPerms group, and the API is never told they
         * have one. This exists so staff can exercise a premium feature without buying it, and the
         * limits it lifts are the ones this server enforces in memory.
         *
         * The server-side limits — how many homes the API will store, how much `/back` budget it
        ...
        public static Entitlements fromJson(JsonObject json) {
            return new Entitlements(
                    optionalString(json, "tierId", null),
                    optionalString(json, "tierName", "None"),
                    optionalInt(json, "level", 0),
                    optionalInt(json, "homeLimit", 0),
                    optionalInt(json, "backUses", 0),
                    optionalInt(json, "lockedChestLimit", 0),
                    optionalBoolean(json, "portableChest"),
                    optionalBoolean(json, "cosmetics"),
                    optionalString(json, "luckPermsGroup", null));
        }
    }
